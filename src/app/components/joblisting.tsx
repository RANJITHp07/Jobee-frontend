...
useMemo(()=>{
  ...
  return (
    <div {...p} className={page ?'box_shadow p-3 mx-10 my-8 rounded-xl md:w-10/12  job md:ml-16 w-full lg:w-full':'box_shadow p-3   my-3 rounded-xl  w-full '} >
      ...
      <div className='flex'>
        ...
        <div className='ml-auto md:ml-auto lg:hidden '>
          <Image src={p.company.logo} width={100} height={100} alt="photo" className='rounded-xl' onClick={()=>{router.push(`/company/${p.company._id}`)}} />
        </div>
        <div className='ml-6 md:ml-auto hidden lg:block'>
        <Image src={p.company.logo} width={100} height={100} alt="photo" className='rounded-xl' onClick={()=>{router.push(`/company/${p.company._id}`)}}/>
        </div>
       </div>
       <div className={`${!page ? "grid" :"first-letter:"}  mb-3 md:mr-96 lg:mr-54`}>
       ...
       {page && <div className='flex text-sm  text-slate-500 my-2 items-center'><DescriptionIcon className='text-sm'/>{p.desc.slice(0,100)}....</div>}
       {
